...
//        http.addFilterBefore(validateCodeFilter, UsernamePasswordAuthenticationFilter.class)
		http.headers().frameOptions().disable().and().formLogin()// 使用表单登录，不再使用默认httpBasic方式
				...
				.and().apply(merryyouSpringSocialConfigurer)// 社交登录
				// .and().rememberMe().tokenRepository(persistentTokenRepository())
				// .tokenValiditySeconds(securityProperties.getRememberMeSeconds()).userDetailsService(userDetailsService)
...
//                .invalidSessionStrategy(invalidSessionStrategy)
				...
				// 之后的登录踢掉之前的登录
				// .expiredSessionStrategy(sessionInformationExpiredStrategy).and().and().logout().logoutUrl("/signOut")//
				// 默认退出地址/logout
				// .logoutSuccessUrl("/")// 退出之后跳转到注册页面
				// .deleteCookies("JSESSIONID")
				.and().authorizeRequests()
				.antMatchers(SecurityConstants.DEFAULT_UNAUTHENTICATION_URL,
						SecurityConstants.DEFAULT_SIGN_IN_PROCESSING_URL_FORM, SecurityConstants.DEFAULT_REGISTER_URL,
						SecurityConstants.DEFAULT_SIGN_IN_PROCESSING_URL_MOBILE,
						SecurityConstants.DEFAULT_SIGN_IN_URL_MOBILE_PAGE, "/register", "/socialRegister", // 社交账号注册和绑定页面
	...
	/**
	 * 去除角色中role_的前缀 表达式需要.access("hasRole('ADMIN')");
	 *
	 * @throws Exception
	 */
	/*
	 * @Bean GrantedAuthorityDefaults grantedAuthorityDefaults() { return new
	 ...
	 * @Bean
	 * 
	 * @ConditionalOnMissingBean(ClassPathTldsLoader.class) public
	 * ClassPathTldsLoader classPathTldsLoader() { return new ClassPathTldsLoader();
	 * }
	 */
}
